$(document).ready(function () {
  ...
  $("#folio").on("change", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#id_col").val(response.id_col);
        $("#nombre_col").val(response.nombre_col);
    ...
    $.ajax({
      ...
      success: function (response) {
        if (Array.isArray(response) && response.length > 0) {
          ...
          Swal.fire(
            "Sin pagos",
            "No se encontraron pagos para este folio",
            "info"
          );
        }
      },
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success && response.id_pres) {
          ...
          $.ajax({
            ...
            success: function (detalleResponse) {
              ...
              if (detalleResponse.success) {
                ...
                Swal.fire(
                  "Error",
                  "Los detalles no se guardaron correctamente",
                  "error"
                );
              }
            },
          ...
          Swal.fire(
            "Error",
            response.message || "Error al guardar encabezado",
            "error"
          );
        }
      },
  ...
  // Inicializar DataTable para vendedores
  var tableVendedor = $("#tablaVendedor").DataTable({
    destroy: true,
    responsive: true,
    paging: false,
    searching: false,
    ...
    columns: [
      ...
      { data: "nombre" },
      {
        data: null,
        render: function (data, type, row) {
          return `<button class="btn btn-sm btn-primary seleccionar-vendedor" 
                                data-id="${row.id_col}" 
                                data-nombre="${row.nombre}">
                                <i class="fas fa-check mr-1"></i>
                        </button>`;
  ...
  // Botón para abrir modal de vendedor
  ...
  function cargarVendedores() {
    $.ajax({
      url: "bd/get_vendedor.php",
      type: "POST", // <-- Usa POST
      dataType: "json",
      success: function (response) {
        console.log("Vendedores cargados:", response);
        if (Array.isArray(response) && response.length > 0) {
          tableVendedor.clear().rows.add(response).draw();
        } else {
          tableVendedor.clear().draw();
        }
      },
      error: function (xhr, status, error) {
  ...
  $("#tablaVendedor").on("click", ".seleccionar-vendedor", function () {
    var id = $(this).data("id");
    var nombre = $(this).data("nombre");

    $("#id_col").val(id);
    $("#nombre_col").val(nombre);
